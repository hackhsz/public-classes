...
def make_shell_context():
    return dict( app=app, db=db, Classes=Class,Skill=Skill,User=User,Mages=Mages)
...
class SkillForm(FlaskForm):
    ...
    section_name=StringField("Name of parent skill_section?")
...
def see_all_skills():
    ...
    return render_template('all_skills.html',all_skills=all_skills,form=form,num_skills=length_skills)
...
#        temp1=str(form.class_number.data)
    ...
    newlist=[]
    #pdb.set_trace()
#    print(repsonsew["items"][0]['pagemap']['metatags'][0]["twitter:url"])
    len_response = len(repsonsew['items'])
    
    for i in range(0,min(4,len_response)):
        dict1= repsonsew["items"][i]['pagemap']['metatags'][0]
        if "twitter:url" in dict1.keys():
            newlist.append((repsonsew["items"][i]["snippet"][0:200],repsonsew["items"][i]['pagemap']['metatags'][0]["twitter:url"]))
        else:
            newlist.append((repsonsew["items"][i]["snippet"][0:200],"url does not exist"))
    flash("Sorry, Google does not support this")
